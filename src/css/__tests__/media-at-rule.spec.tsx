...
describe('@honey-media CSS at-rule', () => {
  ...
  it('should apply styles inside @honey-media(sm:up)', () => {
    ...
    expect(cssText).toBe(
      '.hscn-r1q20b{color:black;}@media screen and (min-width: 768px){.hscn-r1q20b{color:red;}}',
    );
  });
  ...
  it('should support attribute selectors inside @honey-media block', () => {
    const Box = styled('div')`
      @honey-media (xs:down) {
        &[data-open] {
          opacity: 1;
    ...
    customRender(<Box data-testid="media" data-open />);

    const cssText = getRenderedCssText();

    expect(cssText).toContain('@media screen and (max-width: 480px)');
    expect(cssText).toMatch(/\.hscn-[a-z0-9]+\[data-open\]/);
    expect(cssText).toContain('opacity:1');
  });

  it('should support sibling selectors inside @honey-media block', () => {
    const Box = styled('div')`
      @honey-media (sm:down) {
        & + & {
          margin-top: 12px;
    ...
    customRender(
      <>
        <Box data-testid="first" />
        <Box data-testid="second" />
      </>,
    );
    ...
    expect(cssText).toContain('@media screen and (max-width: 768px)');
    expect(cssText).toMatch(/\.hscn-[a-z0-9]+\s*\+\s*\.hscn-[a-z0-9]+/);
